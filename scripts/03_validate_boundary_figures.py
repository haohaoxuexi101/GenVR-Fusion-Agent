#!/usr/bin/env python
"""Validate a trained boundary GMC model against MC CDFs and direction scatters."""
...
def load_model(path: str, device: torch.device):
    ...
    transform = BoundaryTransform.from_dict(ckpt["transform"])
...
def main() -> None:
    ...
    parser.add_argument("--ckpt", type=str, default="outputs/boundary_cfm.pt")
    parser.add_argument("--n", type=int, default=20_000)
    parser.add_argument("--batch-size", type=int, default=4096)
    parser.add_argument("--steps", type=int, default=12)
    parser.add_argument("--outdir", type=str, default="outputs/validation_boundary")
    ...
    for j, (W, H) in enumerate(dims):
        print(f"validating W={W}, H={H}", flush=True)
        batch = generate_boundary_dataset(
            args.n,
            seed=args.seed + j,
            fixed_W=W,
            fixed_H=H,
    ...
    plot_cdf_grid(
        ...
        outdir / "fig_2_5_like_exit_perimeter_cdf.png",
        logx=False,
    )
    plot_cdf_grid(
        ...
        outdir / "fig_2_6_like_path_length_cdf.png",
        logx=True,
    )
    plot_direction_grid(
        mc_targets,
        gmc_targets,
        dims,
        outdir / "fig_2_7_like_exit_direction_scatter.png",
    )
    print(f"saved validation figures to {outdir}")
